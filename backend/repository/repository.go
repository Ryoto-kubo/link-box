...
// linkBoxRepository is a concrete repository that implements repository.
type linkBoxRepository struct {
	*repository
}


func NewLinkBoxRepository(logger logger.Logger, conf *config.Config) Repository {
	...
	return &linkBoxRepository{&repository{db: db}}
}
